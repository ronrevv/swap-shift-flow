...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const user of demoUsers) {
      ...
      // Set role (should happen automatically via trigger, but set explicitly here)
    ...
    const { data: staffShifts } = await supabaseClient
      .from('shifts')
      .select('*')
    ...
    if (staffShifts && staffShifts.length > 0) {
      ...
      const { error: openSwapError } = await supabaseClient
        .from('swap_requests')
        .insert({
      ...
      if (staffShifts.length > 1) {
        const { error: pendingSwapError } = await supabaseClient
          .from('swap_requests')
          .insert({
